...
#Scene Storage
var target_reticle = preload("res://scenes/objects/Test Objects/3D Sprites/Target Reticle/Target_Reticle.tscn")
...
var visible_targets = []
var camera_position
var camera_direction #where camera would be if attached to player
var target_position
var target_direction
var max_cam_target_dotp = 0.65 #angle = 90 - (90 * max_cam_target_dotp)
var main_target
...
func _ready():
	...
	$Rig/cast_position/cast_glow.visible = false
	
	#####For NPC head rig#####
	$Camera_Rig.connect("break_target", self, "_on_break_target")
...
func check_targets_visibility():
	for target in targettable_objects:
		...
		if camera_dotp_target > max_cam_target_dotp: #if target is in view cone
...
#					emit_signal("targets_changed", visible_targets)
#			else:
#				if target == main_target: #if main target in line to camera, stop targetting
#					targetting = false
#					main_target = null
#					emit_signal("focus_target", main_target)
#				visible_targets.erase(target)
...
func draw_target_reticle(target):
	...
	if target.has_node("Reticle"):
		...
		owner.get_node(target.get_name()).add_child(reticle)
		reticle.draw(target)
