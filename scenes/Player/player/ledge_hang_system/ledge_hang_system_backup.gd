...
func calculate_grab_point(Wall_Normal_Raycast, Ledge_Normal_Raycast):
	...
	if Wall_Normal_Raycast.is_colliding() and Ledge_Normal_Raycast.is_colliding() and ledge_normal == Vector3.UP and !Raycast_Ceiling.is_colliding():
		...
		if wall_angle > wall_angle_min or is_equal_approx(wall_angle, wall_angle_min):
			...
			# take the cross product to get axis to rotate around
