...
func look():
	...
	previous_facing_angle.x = calculate_global_x_rotation(get_node_direction(owner.get_node("Rig")))
...
func rotate_camera(input_change):
	...
	focus_angle_global.x = calculate_global_x_rotation(focus_direction)
	facing_angle.y = calculate_global_y_rotation(facing_direction)
	facing_angle.x = calculate_global_x_rotation(facing_direction)
...
#
#	previous_facing_angle.y = calculate_global_y_rotation(facing_direction)
#	previous_facing_angle.x = calculate_global_x_rotation(facing_direction)
...
func center_camera():
	...
	var focus_to_centering_angle = Vector2()
	var facing_angle = Vector2()
	var body_turn_angle = Vector2()
	var facing_turn_angle = Vector2()
	var focus_turn_angle = Vector2()
	...
	#If turning the opposite direction of body, focus will rotate to facing direction
	...
	if is_targetting:
		...
		facing_turn_angle.y = abs(get_y_angle_between(facing_direction, target_direction))
		focus_turn_angle.y = abs(get_y_angle_between(focus_direction, target_direction))
		
		#Determine y direction for focus to rotate to
		if !centered and sign(body_turn_angle.y) != sign(focus_angle.y) and facing_turn_angle.y < focus_turn_angle.y:
...
#	#Set new focus_angle
#	facing_angle = owner.get_node("Rig").get_global_transform().basis.get_euler()
#		#Turning left at degrees > 180
#	if (rotate_angle.y > deg2rad(180)):
#		rotate_angle.y = rotate_angle.y - deg2rad(360)
#	#Turning right at degrees < -180
#	if (rotate_angle.y < deg2rad(-180)):
#		rotate_angle.y = rotate_angle.y + deg2rad(360)
#	focus_angle = facing_angle - centering_angle
...
func calculate_global_x_rotation(direction):
	var test_direction = Vector2(0,1)
	var direction_to = Vector2(direction.y, direction.z)
	var x_rotation
	if direction.z > 0:
		x_rotation = test_direction.angle_to(direction_to)
	else:
		x_rotation = test_direction.angle_to(-direction_to)
	
	return x_rotation #Output is PI > y > -PI
...
func get_y_angle_between(direction1, direction2):
	var test_direction = Vector2(direction1.x, direction1.z)
	var direction_to = Vector2(direction2.x, direction2.z)
	var y_rotation = -test_direction.angle_to(direction_to)
	return y_rotation #Output is PI > y > -PI


func get_x_angle_between(direction1, direction2):
	direction1.x = 0
	direction2.x = 0
	var angle = direction1.angle_to(direction2)
